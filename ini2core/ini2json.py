...
if __name__ == "__main__":
    ...
    # read api type/version first
    id_string = f.readline().split('=')
    if id_string[0].strip().upper() in ['CAPI', 'SAPI']:
        id_type = id_string[0]
    else:
        raise SyntaxError("Could not find API type")
    try:
        id_version = int(id_string[1].strip())
    except ValueError:
        print("Unknown version: \"" + id_string[1].strip())

    cfg.readfp(f)
    f.close()

    config = {}
    config["core_file"] = sys.argv[1];

    list_sects = ['include_files', 'src_files', 'tb_private_src_files',
                  'simulators', 'depend', 'tb_src_files', 'tb_include_files',
                  'patches', 'pre_build_scripts', 'libs']
    ...
    for section in cfg.sections():
        ...
        for name, value in cfg.items(section):
            ...
            if len(config[section][name]) == 1 and (not (name in list_sects)):
